...
const PROFILES: Record<ProfileKey, ProfileResult> = {
  deep_diver: {
    ...
    strategies: [
      "Connect tasks to interests before starting. Establish a real-world link between the subject and something they love.",
  ...
  spark_seeker: {
    ...
    colour: "#F5C518",
    tagline:
      "Attention lights up with novelty, competition, or creativity; fades fast in routine or repetitive tasks.",
    parentMessage:
      "Your child needs variety and stimulation built into learning. Routine kills their focus — novelty unlocks it.",
    strategies: [
  ...
  wanderer: {
    ...
    colour: "#E24B4A",
    tagline:
      "Attention scatters easily across all contexts — regardless of topic, reward, environment, or motivation.",
    parentMessage:
...
export function determineProfile(scores: {
  ...
  if (motivationScore >= 70 && cptBaseScore >= 65) return PROFILES.deep_diver;
  ...
  if (
    fatigueIndex <= -15 ||
    (cptBaseScore >= 55 && parentScore < 55 && recoveryScore < 50)
  )
