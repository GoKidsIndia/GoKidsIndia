...
export function WelcomeScreen({ childrenList, onBegin }: Props) {
  ...
  const activeChild = childrenList.find((c) => c._id === selectedChildId) ?? null;
    ...
    <div className="space-y-6 py-2">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-sm text-gray-500 font-semibold leading-relaxed max-w-sm mx-auto">
          A 4-part assessment to understand your child&apos;s focus, impulse control, and attention patterns.
        </p>
      </div>
            ...
            return (
              ...
              >
                <div
                  ...
                  style={{ background: isSelected ? "#F5C518" : "#F3F4F6", color: isSelected ? "#1A1A1A" : "#6B7280", fontFamily: "var(--font-heading)" }}
                >
                  {child.name.slice(0, 2).toUpperCase()}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-extrabold capitalize text-[#1A1A1A] truncate">{child.name}</p>
                  {age !== null && (
                    <p className="text-xs text-gray-400 font-semibold">
                      {age} years old
                      {bandFromAge(age) && (
                        <span className="ml-1.5 text-[10px] font-extrabold px-1.5 py-0.5 rounded-md" style={{ background: "#FFF9E6", color: "#D4A900" }}>
                ...
                {isSelected && <span className="text-[#F5C518] text-lg">✓</span>}
              </motion.button>
            );
          })}
        </div>
        ...
        <div className="space-y-2">
          {autoBand ? (
            <motion.div
              ...
              style={{ background: "linear-gradient(135deg,#FFFBEA,#FFF3CC)", border: "1.5px solid rgba(245,197,24,0.35)" }}
            >
              <span className="text-xl">✨</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-extrabold" style={{ color: "#92700A", fontFamily: "var(--font-heading)" }}>
                  Age group detected automatically
                </p>
                <p className="text-xs text-[#A07010] mt-0.5 font-semibold">
                  {activeChild.name} is <strong>{childAge} yrs old</strong>: ({BAND_CONFIG[autoBand].ageLabel})
                </p>
              </div>
              <div className="px-3 py-1.5 rounded-xl text-xs font-extrabold shrink-0" style={{ background: "#F5C518", color: "#1A1A1A", fontFamily: "var(--font-heading)" }}>
                Band {autoBand}
              </div>
            </motion.div>
          ) : (
      ...
      {/* What to expect */}
      <div className="rounded-2xl p-4 space-y-2" style={{ background: "#F3F4F6" }}>
        <p className="text-xs font-extrabold uppercase tracking-wider text-gray-400">What to expect</p>
        <div className="grid grid-cols-2 gap-2 text-xs text-gray-600 font-semibold">
          <div className="flex items-start gap-2"><span>Part 1: Digital shape task (child)</span></div>
          <div className="flex items-start gap-2"><span>Part 2: How did it feel? (child)</span></div>
          <div className="flex items-start gap-2"><span>Part 3: Parent observations</span></div>
          <div className="flex items-start gap-2"><span>Part 4: Motivation check</span></div>
        </div>
        <p className="text-[11px] text-gray-400 font-semibold pt-1">Total time: {attentionSpanModalContent.totalTime}</p>
      </div>
