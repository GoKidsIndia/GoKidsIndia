  ...
  return (
    <div className="space-y-6 py-2">
      <div className="flex items-center gap-2">
        ...
        <span className="text-xs text-gray-400 font-semibold">How did it feel?</span>
      </div>
      ...
      <div className="text-center space-y-3">
        ...
        <p className="text-sm text-gray-500 font-semibold leading-relaxed max-w-sm mx-auto">
          Answer honestly — there are no right or wrong answers. This helps us understand how the task felt to you.
        </p>
      </div>

      <div className="space-y-3">
        {[
          { icon: "🤫", text: "Parents — please give the device to your child and give them privacy" },
          { icon: "❓", text: "6 short questions about the shape task you just did" },
          { icon: "⏱️", text: "Takes about 3–4 minutes" },
          { icon: "🔒", text: "Completely private — your honest answers help us understand you better" },
        ].map((item, i) => (
          <div key={i} className="flex items-start gap-3 p-3 rounded-xl" style={{ background: "#FAFAF8" }}>
            <span className="text-xl shrink-0">{item.icon}</span>
            <p className="text-sm font-semibold text-gray-700 leading-relaxed">{item.text}</p>
          </div>
        ))}
      </div>
